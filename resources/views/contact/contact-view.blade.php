  ...
  <div class="container lightBorder rowHighlight">
    ...
    <div class="row">
      <div class="col-sm-6">
        <div class="row">
          ...
          <div
            class="col-xs-8">{{ \App\AppHelper::optionValueFromDB('lkp_salutation', $record->salutation_id) }} {{ $record->first_name.' '.$record->last_name }}</div>
        </div>
      </div>
      ...
      <div class="col-sm-6">
        <div class="row">
          ...
          <div class="col-xs-8"><a
              href="/account/{{ $record->account_id }}">{{ \App\AppHelper::valueFromDB('accounts', $record->account_id) }}</a>
          </div>
        </div>
      </div>
    ...
    <div class="row">
      <div class="col-sm-6">
        <div class="row">
          ...
          <div
            class="col-xs-8">{{ \App\AppHelper::optionValueFromDB('lkp_lead_source', $record->lead_source_id) }}</div>
        </div>
      </div>
    ...
    <div class="row">
      <div class="col-xs-6 col-sm-3 col-md-2 col-md-offset-1 text-nowrap">
        <label for="do_not_call" class="text-right">&nbsp;Do Not Call:</label>&nbsp;&nbsp;<span
          class="glyphicon glyphicon-@if ($record->do_not_call){{ 'ok' }}@endif"></span>
      </div>
      <div class="col-xs-6 col-sm-3 col-md-2 text-nowrap">
        <label for="do_not_email" class="text-right">&nbsp;Do Not Email:</label>&nbsp;&nbsp;<span
          class="glyphicon glyphicon-@if ($record->do_not_email){{ 'ok' }}@endif"></span>
      </div>
      <div class="col-xs-6 col-sm-3 col-md-2 text-nowrap">
        <label for="do_not_fax" class="text-right">&nbsp;Do Not Fax:</label>&nbsp;&nbsp;<span
          class="glyphicon glyphicon-@if ($record->do_not_fax){{ 'ok' }}@endif"></span>
      </div>
      <div class="col-xs-6 col-sm-3 col-md-2 text-nowrap">
        <label for="email_opt_out" class="text-right">&nbsp;Email Opt Out:</label>&nbsp;&nbsp;<span
          class="glyphicon glyphicon-@if ($record->email_opt_out){{ 'ok' }}@endif"></span>
      </div>
      <div class="col-xs-6 col-sm-3 col-md-2 text-nowrap">
        <label for="do_not_call" class="text-right">&nbsp;Fax Opt Out:</label>&nbsp;&nbsp;<span
          class="glyphicon glyphicon-@if ($record->fax_opt_out){{ 'ok' }}@endif"></span>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-sm-6">
        <div class="row">
          ...
          <div class="col-xs-8">{{ $record->street }} <br> {{ $record->city }}, {{ $record->state }}, {{ $record->zip }}
            <br> {{ \App\AppHelper::optionValueCountry($record->country) }}</div>
        </div>
      </div>
      ...
      <div class="col-sm-6">
        <div class="row">
          ...
          <div class="col-xs-8">{{ $record->street_other }} <br> {{ $record->city_other }}, {{ $record->state_other }}
            , {{ $record->zip_other }} <br> {{ \App\AppHelper::optionValueCountry($record->country_other) }}</div>
        </div>
      </div>
    </div>
    ...
    <div class="row">
      <form id="deleteContactForm" method="post" action="/contact/{{ $record->id }}" accept-charset="UTF-8" role="form"
        ...
        <div class="form-group form-group-sm">
          <div class="col-xs-12 text-center">
            ...
            <input type="button" value="Delete" class="btn btn-primary"
                   onClick="if (confirm('Are you sure you want to delete this record?')) document.getElementById('deleteContactForm').submit()">
          </div>
        </div>
      </form>
    </div>
